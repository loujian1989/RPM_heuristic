 ...
 * It was designed to deal with some some general social network.
 */
...
public class AAM_General_SN {
    ...
    AAM_General_SN(int n, int depth, LinkedList<Integer> order1, ArrayList<ArrayList<Integer>> value1, ArrayList<LinkedList<Integer>> linked_value1)
    {
        this.n= n;
        this.depth= depth;
    ...
    private static ArrayList<Integer> DFS (int n, Node node, ArrayList<Integer> array, int depth, ArrayList<ArrayList<Integer>> value, ArrayList<LinkedList<Integer>> linked_value, int rank)
    {
        ...
        if(rank==0) {
            ...
            while (count == true && node.order.size() > 1) {
                for (int i = 1; i <= n; i++) {
                    if (!node.preference.get(i).isEmpty()) //Then we could know whether the player is available
                        flag_profile[i] = true;
                    else
                        flag_profile[i] = false;
                }
        ...
        while(node.preference.get(proposer).isEmpty()) //if the preference of the proposer is empty, then we should remove the player from the order list
        {
        ...
        ArrayList<Integer> reject_array = new ArrayList<Integer>(n+1); //denote the value each player could get by accepting the offer
        //accept_array.clear();
        reject_array.addAll(array);

        if(!order_reject.isEmpty()) {
            Integer newplayer1 = order_reject.getFirst();
            Node reject_node = new Node(newplayer1, array, order_reject, reject_preference);

            reject_array = DFS(n, reject_node, array, tmp_depth_1, value, linked_value, rank);
        }
