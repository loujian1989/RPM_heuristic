...
public class Team_Form_Pranav_V2 {
    ...
    Team_Form_Pranav_V2(int n, int depth, LinkedList<Integer> order1, ArrayList<ArrayList<Integer>> value1, ArrayList<LinkedList<Integer>> linked_value1)
    {
        this.n= n;
        this.depth= depth;
    ...
    public ArrayList<Integer> ARM ()
    {
        ...
        Integer[] flag= new Integer[n+1]; //to denote rejected players previously
        for(int i=1; i<=n; i++) //at the very first, never rejected by any players
            flag[i]=-1;

        array= DFS(n, root_node, array, depth, value, linked_value, 0, flag);

        return array;

    }

    private static ArrayList<Integer> DFS (int n, Node node, ArrayList<Integer> array, int depth, ArrayList<ArrayList<Integer>> value, ArrayList<LinkedList<Integer>> linked_value, int rank, Integer[] reject_flag)
    {
        ...
        Integer[] current_flag= new Integer[n+1];
        for(int i=1; i<=n; i++)
        ...
        if(current_flag[proposer]>=0)
        {
            ...
            {
                LinkedList<Integer> order_tmp = new LinkedList<Integer>(); //the order if the receiver accept the offer
                order_tmp.clear();
                order_tmp.addAll(node.order);
                rank=0;
                order_tmp.removeFirst();

                ArrayList<LinkedList<Integer>> tmp_preference= new ArrayList<LinkedList<Integer>>();
                tmp_preference.clear();

                LinkedList<Integer> non_sense= new LinkedList<>();
                non_sense.add(1);
                tmp_preference.add(non_sense); //just to fill the index 0, non-sense here
                for(int k=1; k<=n; k++)
                {
                    LinkedList<Integer> tmp1_list= new LinkedList<>();
                    tmp1_list.addAll(node.preference.get(k));
                    tmp_preference.add(tmp1_list);
                }
                for(int k=1; k<=n; k++) {
                    tmp_preference.get(k).remove(proposer);
                }

                int tmp_depth_2= depth;
                tmp_depth_2--;
                if(tmp_depth_2>=order_tmp.size())
                    tmp_depth_2=order_tmp.size();

                Integer newplayer_tmp=order_tmp.getFirst();
                Node tmp_node= new Node(newplayer_tmp, array, order_tmp, tmp_preference);

                ArrayList<Integer> tmp_array= DFS(n, tmp_node, array, tmp_depth_2, value, linked_value, 0, current_flag);
        ...
        if(!order_accept.isEmpty())
        {
            ...
            accept_array = DFS(n, accept_node, accept_array, tmp_depth, value, linked_value, 0, current_flag);
        }
        ...
        if(rank== node.preference.get(proposer).size()-1) // will revise it later
        {
            rank=0;
        ...
        ArrayList<Integer> reject_array= DFS(n, reject_node, array, tmp_depth_1, value, linked_value, rank, tmp_flag);
